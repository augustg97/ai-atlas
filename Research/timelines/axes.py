  ...
  "changelog": [
    ...
    {"version": "r8-2026-08-20", "date": "2026-08-20",
     "change": "r8b - the argmax stops enumerating, and the test that would "
               ...
               "is 102 minutes a call. Three readings now exist and they "
               "agree inside 9%: 102 projected from throughput, 95.1 measured "
               "through exemplars(k=5), and 93.3 measured on mainline itself, "
               "both wall-clock and both from processes that held the old code "
               "for their whole lives. THE THIRD IS THE PER-CALL FIGURE, "
               "because exemplars contains one mainline call plus a "
               "4,000-sample ensemble and five world-line instantiations, so "
               "it must exceed the bare call and the two bracket correctly. "
               "93.3 minutes a call, 3.1 hours for the two the emit makes. The "
               "same probe timed bands at 0.02s, which rules the rest of the "
               "emit out rather than merely leaving it unaccused: an "
               "extrapolation names a suspect, a direct timing convicts one, "
               "and a negative control on everything else is what turns this "
               "is where the time COULD be into this is where it IS. The run "
               "was stopped twice before anyone read the cause. It is now "
               ...
               "worse than a slow one. AND THE COMPLETE SPACE WAS ENUMERATED "
               "TO SETTLE IT: all 120,960,000 cells, 5,727 seconds, returning "
               "the same line and the same probability to twelve digits, a "
               "speedup of 10,785x. That run is a fourth wall-clock reading of "
               "the old cost at 95.5 minutes, and the four are coherent - 93.3 "
               "for the bare call, 95.1 through exemplars, 95.5 with a "
               "per-cell comparison on top, and 102 projected, the outlier. "
               "NOTE WHAT THAT RUN DOES NOT SAY: it enumerates the seed "
               "registry, so it establishes that branch and bound equals "
               "enumeration, not that the published line is optimal on the "
               "widened space the emit uses. A separate neighbourhood test - "
               "radius 2 over 1,179 neighbours, and 300,000 draws whose best "
               "was the published line - speaks to that, and the two questions "
               "are only joined while the seed and widened argmaxes agree, "
               "which is now asserted. SEPARATELY, worldlines._selftest HAD "
