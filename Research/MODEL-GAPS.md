...
| D3 | **P1** | **`events.explicit_pct` is enforced against a stale composition and blocks publication.** Baseline 91.7 frozen 2026-08-28 at n=1,825; tolerance 1.0; tonight n=2,456 at 90.676% → rounds to 90.7 → passed on `90.7 < 90.7` being False, margin **nil**. Incoming stream runs 77.3% explicit (events ≥ 2026-09-02), so one ordinary night takes it to 90.6 and the gate refuses. **Not a data defect** — all 11 September fall-backs verified correct under rule 12 (8 genuinely undated; 3 carry publication dates or background references the extractor rightly declined). Fix: split the metric by date-kind (measure explicit rate among events that *could* carry an event date; track the undateable share separately) rather than re-baselining, which re-spends the ratchet's band and will recur. Window-label defect class, 5th sighting. | audit_all.py, baselines.json | nightly 2026-09-04 §0 |
| D4 | **P1** | **The published `mainline.wl` medoid is sampling noise night-over-night.** 09-03 → 09-04 it moved on 4 of 11 axes (R4→R2, S3→S1, P3→P2, G2→G1) and its p halved, while the deterministic `argmax` was identical on all 11 (p −0.345%) and total marginal movement was 0.00805. Measured, not inferred: 12 resamples of 1,000 from tonight's own 2,000-line ensemble gave **three distinct medoid lines** differing on 0/4/5 axes — one of them *exactly* last night's published medoid. Validity check: min-summed-Hamming medoid reproduces the published `wl` exactly on the full 2,000. `agree` 0.296 = a broad flat cloud whose centre is a near-tie. Fix: carry the argmax as the series line, or raise n until the medoid is stable, or label `wl` a sample so it is not read as news. | forecast_emit.py, worldlines.py, app headline | nightly 2026-09-04 §2 |
| D5 | P2 | **`URL_DF_MAX` inverts on a big story.** The two GPT-6 Astra release reports cleared `INCIDENT_SIM_URL` (sim 0.2342 > 0.20) and shared a non-digest article, but that article's document frequency was 4 in the 23-event fresh set — above `URL_DF_MAX=2` — so it was discarded as an aggregator and the pair fell to the 0.45 fallback and did not merge. All four citing events are about that one story. Document frequency cannot separate *an aggregator covering many stories* from *one story generating many events*, and fails hardest where merging matters most. Counterfactual at `URL_DF_MAX=4`: `ev-frontier-release` Σmag 0.0015536 → 0.0013183, i.e. **17.8% over-application** tonight (√n/n spread absorbs the rest, as designed). Second-order: IDF and udf are both computed over one night's fresh set, so merge behaviour depends on how busy the night was. | nightly_update.py `same_incident` | nightly 2026-09-04 §3 |
